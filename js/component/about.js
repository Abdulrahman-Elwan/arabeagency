...
export default function displayAboutSection() {
    ...
    const accordions = accordionData.map(acc => {
        // نحدد الكلاس حسب حالة الـ active
        let imgClass = acc.active ? "imge" : "";

        return `
            <div class="accordion mb-0 block ${acc.active ? 'active-block' : ''}">
                <div class="acc-btn p-0" dir="rtl">
                    <div class='d-flex align-items-center gap-5'>
                        <img src="images/icon/accordion.svg" style="width: 30px;" class="${imgClass}" alt="${acc.title}"/>
                        <h3 class='pt-4'>${acc.title}</h3>
                    </div>
                </div>
    ...
    accordionBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            document.querySelectorAll('.accordion.block').forEach(acc => {
                ...
                acc.querySelector("img").classList.remove("imge");
            });

            // افتح الحالي لو مكنش مفتوح
            if (!isActive) {
                currentAccordion.classList.add('active-block');
                currentAccordion.querySelector("img").classList.add("imge");
            }
        });
    });
}
