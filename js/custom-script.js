(function($) {
	...
	function openMenu(){
		...
		$('.nav-toggler .bar-three').addClass('d-none');
		$('.nav-toggler .bar').addClass('barClose');
	}
	function hiddenMenu(){
		...
		$('.nav-toggler .bar-three').removeClass('d-none');
		$('.nav-toggler .bar').removeClass('barClose');
	}
	if($('.main-header-bar .nav-toggler .toggler-btn').length){
		$('.main-header-bar .nav-toggler .toggler-btn').on('click', function(e) {
			e.preventDefault();
			if(!document.querySelector('.main-nav-outer').classList.contains('now-visible')){
				openMenu();
			}else{
				hiddenMenu();
			}
